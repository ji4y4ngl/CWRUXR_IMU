/****************************************************************
 * This code is based on Example9_DMP_MultipleSensors.ino from the Sparkfun ICM 20948 Arduino Library and references 
 * said library at https://github.com/sparkfun/SparkFun_ICM-20948_ArduinoLibrary
 *
 * Author: JiaYang Lin @CWRU_Interactive_Commons
 * Initialize the DMP based on the TDK InvenSense ICM20948_eMD_nucleo_1.0 example-icm20948
 * Sparkfun code Authors: Paul Clark, April 25th, 2021, Owen Lyke @ SparkFun Electronics, April 17 2019
 * 
 ...
 * ** Make sure that the ICM_20948_C.h file in the library has DMP enabled
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ((data.header & DMP_header_bitmap_Quat6) > 0) // Check for orientation data (Quat9)
    {
      ...
      SERIAL_PORT.print(F("Rotation Quaternion[ "));
    ...
    {
      float acc_x = (float)data.Raw_Accel.Data.X/1000*9.80665; // Extract the raw accelerometer data
      float acc_y = (float)data.Raw_Accel.Data.Y/1000*9.8066;
      float acc_z = (float)data.Raw_Accel.Data.Z/1000*9.8066;

      SERIAL_PORT.print(F("Acc (m/s^2) [ "));
    ...
    {
      float x = (float)data.Raw_Gyro.Data.X/0.017453; // Extract the raw gyro data
      float y = (float)data.Raw_Gyro.Data.Y/0.017453;
      float z = (float)data.Raw_Gyro.Data.Z/0.017453;

      SERIAL_PORT.print(F("Gyr (Radians) [ "));
      SERIAL_PORT.print(x);
      SERIAL_PORT.print(F(","));
      SERIAL_PORT.print(y);
      SERIAL_PORT.print(F(","));
    ...
    if ((data.header & DMP_header_bitmap_Compass) > 0) // Check for Compass
    {
      ...
      SERIAL_PORT.print(F("Compass (uT) [ "));
      SERIAL_PORT.print(x);
      SERIAL_PORT.print(F(","));
      SERIAL_PORT.print(y);
      SERIAL_PORT.print(F(","));
